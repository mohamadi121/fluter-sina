...
| Buyer commerce | public-market discovery, bookmarks, cart, order/payment/wallet basics | product search, comment update, discount application UX |
...
| 4 | Buyer: browsing | bookmarks page rebuilt on BookmarkCubit; customer discovery is now only the real public-market search/list (the fake order/stat tabs were removed); public mobile routes are mounted under `/api/v1/` | DONE. Gap: standalone product search does not exist in backend |
| 5 | Buyer: cart→payment | `/shopping_cart` uses CartScreen; add-item/payment/wallet top-up use real contracts | DONE. Deferred: cart discount application UX |
