...
class CustomerBloc extends Bloc<CustomerEvent, CustomerState> {
  ...
  void _onGetCusomerRequest(
    ...
    try {
      final res = Success(true); // TODO: Implement customer request retrieval
      if (res is Success) {
  ...
  void _onGetCusomerOrders(
    ...
    emit(state.copyWith(status: const UiLoading()));
    try {
      final res = Success(<dynamic>[]); // TODO: Implement customer orders retrieval
      if (res is Success) {
        emit(state.copyWith(status: const UiSuccess()));
      }
    } catch (e) {
  ...
  void _onGetCusomerStors(GetCusomerStors event, Emitter<CustomerState> emit) {
    emit(state.copyWith(status: const UiLoading()));
    try {
      final res = Success(<dynamic>[]); // TODO: Implement customer stores retrieval
      if (res is Success) {
        emit(state.copyWith(status: const UiSuccess()));
      }
    } catch (e) {
  ...
  void _onTrackPayment(TrackPayment event, Emitter<CustomerState> emit) {
    emit(state.copyWith(status: const UiLoading()));
    try {
      final res = Success(true); // TODO: Implement payment tracking
      if (res is Success) {
        emit(state.copyWith(status: const UiSuccess()));
      }
    } catch (e) {
  ...
  void _onAcceptPayment(AcceptPayment event, Emitter<CustomerState> emit) {
    emit(state.copyWith(status: const UiLoading()));
    try {
      final res = Success(true); // TODO: Implement payment acceptance
      if (res is Success) {
        emit(state.copyWith(status: const UiSuccess()));
      }
    } catch (e) {
